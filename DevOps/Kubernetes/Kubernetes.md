...
[[Kubernetes#^3aef8d|К содержанию]]
## Как работает:
1. **Мастер-узел** (мастер-нода) управляет кластером и включает компоненты API Server, Scheduler и Controller Manager, которые отслеживают состояние кластера и распределяют поды на узлы.
